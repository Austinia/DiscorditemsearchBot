...
client.on("message", async (msg) => {
  ...
  if (msg.content == "!도움말" || msg.content == "!help") {
    let HelpEmbed = new Discord.MessageEmbed()
      ...
      .setThumbnail(
        (url =
          "https://img.finalfantasyxiv.com/lds/h/9/_Huf58epDlt9vXiO8IIfPXxtXI.png")
      )
      .addFields(
        {
          name: "!도움말 || !help",
          value: "도움말을 가져옵니다. 바로 이 창을 띄웁니다.",
        },
        {
          name: "!s:닉네임",
          value:
            "입력한 닉네임을 토대로 넓은 기준으로 검색합니다.\n(정확한 서버를 모르거나 기억하는 닉네임이 애매할때 사용 하면 좋습니다.)\nex) !s:Rosen Kranz",
        },
        {
          name: "!d:닉네임:서버",
          value:
        ...
        // },
        {
          name: "!f:자유부대명:서버",
          value:
            "해당 자유부대의 부대원들을 찾아줍니다.\nex) !f:Equilibrium:Phoenix",
        },
        {
          name: "!b:닉네임:서버",
          value:
            "해당 캐릭터의 친구리스트를 불러옵니다.(공개 설정이 되어있는 경우만)\nex) !b:Rosen Kranz:Pandaemonium",
        },
        {
          name: "**@@@@@@@@검색시 주의사항@@@@@@@@**",
          value:
            "**대문자 소문자 와 띄어쓰기 까지 정확히 입력해야 검색가능해쿠뽀!**\n**세미콜론; 이 아니고 콜론: 이야 쿠뽀!**",
        }
      )
      .setFooter(
        "FFXIV Searching Bot",
  ...
  if (command[0] == "d" && command[1] && command[2]) {
    ...
    //둘다 안적었을때
    msg.channel.send(
      "자세히 알고 싶은 닉네임이랑 서버를 같이입력해줘쿠뽀! ex)!d:Rosen Kranz:Pandaemonium"
    );
  }
  /*-------------------------------------------------------*/
  ...
  if (command[0] == "f" && command[1] && command[2]) {
    ...
    //둘다 안적었을때
    msg.channel.send(
      "알고 싶은 부대이름이랑 서버를 같이입력해줘쿠뽀! ex)!d:Rosen Kranz:Pandaemonium"
    );
  }
  /*-------------------------------------------------------*/
  if (command[0] == "b" && command[1] && command[2]) {
    ...
    //둘다 안적었을때
    msg.channel.send(
      "친구들을 자세히 알고 싶은 닉네임이랑 서버를 같이입력해줘쿠뽀! ex)!d:Rosen Kranz:Pandaemonium"
    );
  }
});
/*----------------------------------------------------------*/
const loadingstuff = async (command, msg) => {
  try {
    const responce = await axios.get(
      `https://xivapi.com/character/search?name=${command[1]}&private_key=${config.API_KEY}`
    );
    ...
    for (let i = 0; i < searchinglist.length; i++) {
      ...
      let serverName = why[0].trim(); // "Pandaemonium" 앞 뒤 공백제거
      let dcName = getDC(searchinglist[i].Server);
      let LoadChar = new Discord.MessageEmbed()
        ...
        .addFields(
          {
            name: `${searchinglist[i].Name}`,
            value: `언어 = ${searchinglist[i].Lang}\n서버 = ${searchinglist[i].Server}`,
          },
          {
            name: "로드스톤 프로필 링크(영어)",
            value: `https://na.finalfantasyxiv.com/lodestone/character/${searchinglist[i].ID}/`,
            inline: true,
          },
          {
            name: "FFLOG 링크",
            value: `https://www.fflogs.com/character/${dcName}/${serverName}/${mURL_Name}`,
            inline: true,
          }
        )
        .setFooter(
    ...
    if (error == "Error: Request failed with status code 404") {
      msg.channel.send(
        `검색 결과가 없어쿠뽀, 오타, 대문자, 서버 와 닉네임을 확인해줘쿠뽀`
      );
      ...
      msg.channel.send(
        "로드스톤이 점검 중 인것 같아쿠뽀! 점검이 끝나고 시도해봐쿠뽀!"
      );
    } else {
      console.error(error);
      msg.channel.send(`오류가 생겼어쿠뽀! 개발자에게 문의해줘쿠뽀 : ${error}`);
...
const loadingstuffdetail = async (command, msg) => {
  try {
    ...
    for (let i = 0; i < obj.length; i++) {
      ...
      let serverName = why[0].trim(); // "Pandaemonium" 앞 뒤 공백제거
      if (something == command[1] && serverName == command[2]) {
        searchinglist = obj[i];
      }
    }
    if (searchinglist.length == 0) {
    ...
    const IDresponce = await axios.get(
      `https://xivapi.com/character/${searchinglist.ID}?&private_key=${config.API_KEY}&data=AC,FR,FC,FCM,PVP`
    );
    ...
    let Fcn = IDobj.FreeCompanyName;
    let dcName = getDC(IDobj.Server);
    let Fct = "";
    if (Fcn == null) {
      Fcn = "부대없음";
      Fct = "없어용";
    ...
    let thisman = new Discord.MessageEmbed() //기본 정보 및 전투 클래스
      ...
      .addFields(
        ...
        {
          name: "로드스톤 블로그",
          value: `https://na.finalfantasyxiv.com/lodestone/character/${IDobj.ID}/blog/`,
        },
        {
          name: "FFLOG 링크",
          value: `https://www.fflogs.com/character/${dcName}/${IDobj.Server}/${URL_Name}`,
        },
        {
          name: `나이트/검술사`,
          value: `${IDobj.ClassJobs[0].Level}`,
          inline: true,
        },
        {
          name: `전사/도끼술사`,
          value: `${IDobj.ClassJobs[1].Level}`,
          inline: true,
        },
        {
          name: `암흑기사`,
          value: `${IDobj.ClassJobs[2].Level}`,
          inline: true,
        },
        {
          name: `건브레이커`,
          value: `${IDobj.ClassJobs[3].Level}`,
          inline: true,
        },
        {
          name: `백마도사/환술사`,
          value: `${IDobj.ClassJobs[4].Level}`,
          inline: true,
        },
        {
          name: `학자/비술사`,
          value: `${IDobj.ClassJobs[5].Level}`,
          inline: true,
        },
        {
          name: `점성술사`,
          value: `${IDobj.ClassJobs[6].Level}`,
          inline: true,
        },
        {
          name: `현자`,
          value: `${IDobj.ClassJobs[7].Level}`,
          inline: true,
        },
        {
          name: `몽크/격투가`,
          value: `${IDobj.ClassJobs[8].Level}`,
          inline: true,
        },
        {
          name: `용기사/창술사`,
          value: `${IDobj.ClassJobs[9].Level}`,
          inline: true,
        },
        {
          name: `닌자/쌍검사`,
          value: `${IDobj.ClassJobs[10].Level}`,
          inline: true,
        },
        {
          name: `사무라이`,
          value: `${IDobj.ClassJobs[11].Level}`,
          inline: true,
        },
        {
          name: `리퍼`,
          value: `${IDobj.ClassJobs[12].Level}`,
          inline: true,
        },
        {
          name: `음유시인/궁술사`,
          value: `${IDobj.ClassJobs[13].Level}`,
          inline: true,
        },
        { name: `기공사`, value: `${IDobj.ClassJobs[14].Level}`, inline: true },
        { name: `무도가`, value: `${IDobj.ClassJobs[15].Level}`, inline: true },
        {
          name: `흑마도사/주술사`,
          value: `${IDobj.ClassJobs[16].Level}`,
          inline: true,
        },
        {
          name: `소환사/비술사`,
          value: `${IDobj.ClassJobs[17].Level}`,
          inline: true,
        },
        {
          name: `적마도사`,
          value: `${IDobj.ClassJobs[18].Level}`,
          inline: true,
        },
        {
          name: `청마도사`,
          value: `${IDobj.ClassJobs[19].Level}`,
          inline: true,
        }
      )
      .setFooter(
    ...
    let thisman2 = new Discord.MessageEmbed() // 채제작 클래스 및 보즈야 에우레카 레벨
      ...
      .setThumbnail(`${IDobj.Avatar}`)
      .addFields(
        { name: "목수", value: `${IDobj.ClassJobs[20].Level}`, inline: true },
        {
          name: "대장장이",
          value: `${IDobj.ClassJobs[21].Level}`,
          inline: true,
        },
        {
          name: "갑옷제작사",
          value: `${IDobj.ClassJobs[22].Level}`,
          inline: true,
        },
        {
          name: "보석공예가",
          value: `${IDobj.ClassJobs[23].Level}`,
          inline: true,
        },
        {
          name: "가죽공예가",
          value: `${IDobj.ClassJobs[24].Level}`,
          inline: true,
        },
        { name: "재봉사", value: `${IDobj.ClassJobs[25].Level}`, inline: true },
        {
          name: "연금술사",
          value: `${IDobj.ClassJobs[26].Level}`,
          inline: true,
        },
        { name: "요리사", value: `${IDobj.ClassJobs[27].Level}`, inline: true },
        { name: "광부", value: `${IDobj.ClassJobs[28].Level}`, inline: true },
        { name: "원예가", value: `${IDobj.ClassJobs[29].Level}`, inline: true },
        { name: "어부", value: `${IDobj.ClassJobs[30].Level}`, inline: true },
        { name: "보즈야 레벨", value: `${Ibozjan}`, inline: true },
        {
          name: "엘레멘탈 레벨",
          value: `${IDobj.ClassJobsElemental.Level}`,
          inline: true,
        }
      )
      .setFooter(
    ...
    if (FCobj !== null) {
      ...
      if (FCobj.Estate.Name !== null) {
        ...
        let thisman3 = new Discord.MessageEmbed()
          ...
          .setThumbnail(
            (url =
              "https://img.finalfantasyxiv.com/lds/pc/global/images/itemicon/c5/c5253b7223d6842a8c0a1aec648ef13fc7a7d1de.png?n5.58")
          )
          .addFields({
            name: `부대집 이름 : ${FCobj.Estate.Name}`,
            value: `위치 : ${FCobj.Estate.Plot}`,
          });
        thisman3.setFooter(
          "로드스톤 API 정보",
          "https://img.finalfantasyxiv.com/lds/h/9/_Huf58epDlt9vXiO8IIfPXxtXI.png"
        ...
        for (let i = 0; i < FCMobj.length; i++) {
          ...
          let member = new Discord.MessageEmbed()
            ...
            .addFields(
              {
                name: FCMobj[i].Name,
                value: `계급 : ${FCMobj[i].Rank}\n로드스톤 캐릭터 정보 링크(영어) : https://na.finalfantasyxiv.com/lodestone/character/${mID}/`,
                inline: false,
              },
              {
                name: "로드스톤 블로그 링크",
                value: `https://na.finalfantasyxiv.com/lodestone/character/${mID}/blog/`,
                inline: true,
              },
              {
                name: "FFLOG LINK (JP only)",
                value: `https://www.fflogs.com/character/jp/${IDobj.Server}/${mURL_Name}`,
                inline: true,
              }
            )
            .setFooter(
        ...
        for (let i = 0; i < FRobj.length; i++) {
          ...
          let friend = new Discord.MessageEmbed()
            ...
            .addFields(
              {
                name: FRobj[i].Name,
                value: `로드스톤 캐릭터 정보 링크(영어) : https://na.finalfantasyxiv.com/lodestone/character/${FRobj[i].ID}/`,
              },
              {
                name: "로드스톤 블로그 링크",
                value: `https://na.finalfantasyxiv.com/lodestone/character/${FRobj[i].ID}/blog/`,
                inline: true,
              },
              {
                name: "FFLOG LINK (JP only)",
                value: `https://www.fflogs.com/character/jp/${serverName}/${mURL_Name}`,
                inline: true,
              }
            )
            .setFooter(
...
const loadingfreecompanymember = async (command, msg) => {
  try {
    const responce = await axios.get(
      `https://xivapi.com/freecompany/search?name=${command[1]}&server=${command[2]}&private_key=${config.API_KEY}`
    );
    const obj = responce.data.Results;
    console.log("obj ready");
    for (let i = 0; i < obj.length; i++) {
      let something = obj[i].Name;
      let where = obj[i].Server.split(" ")[0];
    ...
    const IDresponce = await axios.get(
      `https://xivapi.com/freecompany/${companylist.ID}?&private_key=${config.API_KEY}&data=FCM`
    );
    const FCobj = IDresponce.data.FreeCompany;
    const FCMobj = IDresponce.data.FreeCompanyMembers;
    for (let i = 0; i < FCMobj.length; i++) {
      const mname_temp = FCMobj[i].Name.split(" ");
      const mURL_Name = mname_temp[0] + "%20" + mname_temp[1];
      const dcName = getDC(FCMobj[i].Server);
      let member = new Discord.MessageEmbed()
        .setColor("#00ff9d")
        ...
        .addFields(
          {
            name: FCMobj[i].Name,
            value: `계급 : ${FCMobj[i].Rank}\n로드스톤 캐릭터 정보 링크(영어) : https://na.finalfantasyxiv.com/lodestone/character/${FCMobj[i].ID}/`,
            inline: false,
          },
          {
            name: "로드스톤 블로그 링크",
            value: `https://na.finalfantasyxiv.com/lodestone/character/${FCMobj[i].ID}/blog/`,
            inline: true,
          },
          {
            name: "FFLOG 링크",
            value: `https://www.fflogs.com/character/${dcName}/${command[2]}/${mURL_Name}`,
            inline: true,
          }
        )
        .setFooter(
...
const getDC = (server) => {
  ...
  const oc = ["Bismarck", "Ravana", "Sephirot", "Sophia", "Zurvan"];

  if (jp.includes(server)) return "jp";
  if (na.includes(server)) return "na";
  if (eu.includes(server)) return "eu";
  if (oc.includes(server)) return "oc";
};
/*----------------------------------------------------------*/
